  ...
  <nav class="fixed w-full z-50 top-0 left-0 border-b border-purple-900 bg-[#1B1226]">
    <div class="max-w-7xl mx-auto px-6 h-16 flex items-center gap-6">
      ...
      <div class="hidden md:flex items-center gap-6">
        ...
        <!-- Catalogo: visible para todos -->
        <router-link to="/"
          class="text-purple-300 hover:text-white transition font-medium text-sm">
          Catalogo
        </router-link>

        <!-- Mi Biblioteca: solo clientes logueados -->
        ...
        <router-link v-if="authStore.isAuthenticated && authStore.isAdmin"
          to="/dashboard"
          class="text-purple-300 hover:text-white transition font-medium text-sm">
      ...
      <div class="hidden md:flex items-center gap-3 ml-auto">
        ...
        <div v-if="authStore.isAuthenticated && authStore.isCliente" class="relative inline-block">
          ...
          <span v-if="carritoStore.cantidadLibros > 0"
            class="absolute -top-1 -right-1 bg-red-600 text-white text-[10px] font-bold w-5 h-5 flex items-center justify-center rounded-full border-2 border-[#1B1226] animate-bounce-once">
        ...
        <div v-if="!authStore.isAuthenticated" class="flex gap-3">
          <router-link to="/login"
            class="flex items-center gap-1 px-4 py-1.5 border border-purple-400 rounded-lg text-purple-400 hover:bg-purple-900 transition text-sm font-semibold">
            <i class="pi pi-sign-in"></i>
            Iniciar Sesion
          </router-link>
          <router-link to="/registro"
            class="flex items-center gap-1 px-4 py-1.5 bg-purple-500 hover:bg-purple-700 transition rounded-lg text-white text-sm font-semibold">
            <i class="pi pi-user-plus"></i>
            Registrarse
          </router-link>
        </div>

        <!-- Logueado: perfil y cerrar sesion -->
        <div v-else class="flex items-center gap-4 border-l border-purple-900 pl-3 ml-2">
          ...
          <button @click="authStore.logout()"
            class="flex items-center gap-1 px-4 py-1.5 bg-purple-700 hover:bg-purple-900 transition rounded-lg text-white text-sm font-semibold">
    ...
    <transition name="slide">
      <div v-if="menuAbierto" class="md:hidden border-t border-purple-900 px-6 py-6 flex flex-col gap-4 bg-[#1B1226]">
        ...
        <!-- Catalogo: visible para todos -->
        <router-link to="/" @click="menuAbierto = false"
          class="text-purple-300 hover:text-white transition font-medium flex items-center gap-2">
          <i class="pi pi-th-large"></i> Catalogo
        </router-link>

        <!-- Mi Biblioteca: solo clientes -->
        <router-link v-if="authStore.isAuthenticated && authStore.isCliente"
          to="/mi-biblioteca" @click="menuAbierto = false"
          class="text-purple-300 hover:text-white transition font-medium flex items-center gap-2">
        ...
        <!-- Panel de Administrador: solo admin -->
        <router-link v-if="authStore.isAuthenticated && authStore.isAdmin"
          to="/dashboard" @click="menuAbierto = false"
          class="text-purple-300 hover:text-white transition font-medium flex items-center gap-2">
          Panel de Administrador
        </router-link>
        ...
        <div v-if="authStore.isAuthenticated && authStore.isCliente" class="relative inline-block">
          ...
          <span v-if="carritoStore.cantidadLibros > 0"
            class="absolute -top-1 bg-red-600 text-white text-[10px] font-bold w-5 h-5 flex items-center justify-center rounded-full border-2 border-[#1B1226] animate-bounce-once">
            {{ carritoStore.cantidadLibros }}
          </span>
        </div>

        <!-- Toggle tema -->
        <button @click="toggleTema" class="text-purple-300 hover:text-white hover:bg-purple-900 transition p-2 rounded-full flex items-center w-fit">
          <i :class="oscuro ? 'pi pi-sun' : 'pi pi-moon'"></i>
        </button>

        <!-- No logueado -->
        <div v-if="!authStore.isAuthenticated" class="flex gap-3">
          <router-link to="/login" @click="menuAbierto = false"
            class="flex items-center gap-1 px-4 py-1.5 border border-purple-400 rounded-lg text-purple-400 hover:bg-purple-900 transition text-sm font-semibold">
            <i class="pi pi-sign-in"></i>
            Iniciar Sesion
          </router-link>
          <router-link to="/registro" @click="menuAbierto = false"
            class="flex items-center gap-1 px-4 py-1.5 bg-purple-500 hover:bg-purple-700 transition rounded-lg text-white text-sm font-semibold">
            <i class="pi pi-user-plus"></i>
            Registrarse
          </router-link>
        ...
        <!-- Logueado -->
        <div v-else class="flex items-center gap-4 border-t border-purple-900 pt-4">
          <div class="flex items-center border-2 border-black bg-gray-400 overflow-hidden shadow-[4px_4px_0px_0px_rgba(0,0,0,1)]">
            <div class="px-2 py-1 border-r-2 border-black bg-gray-200">
              <i class="pi pi-user text-black text-sm"></i>
            </div>
...
.animate-bounce-once {
  animation: bounceOnce 0.5s cubic-bezier(0.36, 0, 0.66, -0.56) 1;
}
